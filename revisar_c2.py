...
function agrupar(filas){
  ...
  for(const f of filas){
    ...
    // Veredicto del corrector: 'validado' tiene prioridad sobre 'descartado'
    if(f.revisado_ia === 'validado' && t.revIa !== 'validado'){
      t.revIa = 'validado'; t.motivoIa = f.motivo_ia || '';
    }else if(f.revisado_ia === 'descartado' && !t.revIa){
      t.revIa = 'descartado'; t.motivoIa = f.motivo_ia || '';
    }
  }
  return turnos;
}
...
def compactar(con, sesion_id, solo_ids=None):
    ...
    for g in grupos:
        ...
        if "revisado_ia" in base.keys():
            # 'validado' tiene prioridad; si no, el 'descartado' del grupo
            rev = mot = None
            for f in g:
                rr = _campo(f, "revisado_ia")
                if rr == "validado":
                    rev, mot = "validado", _campo(f, "motivo_ia")
                    break
                if rr == "descartado" and rev is None:
                    rev, mot = "descartado", _campo(f, "motivo_ia")
