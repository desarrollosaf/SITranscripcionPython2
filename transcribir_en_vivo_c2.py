...
def abrir_db(ruta):
    ...
    # Migración para bases creadas antes de existir estas columnas.
    #   fuente     : 'rapido' (1ª pasada en vivo) | 'corregido' (2ª pasada)
    #   revisado_ia: 'validado' | 'descartado' (veredicto del corrector)
    #   motivo_ia  : explicación legible del veredicto
...
def main():
    ...
    def _marcar_descartado(t, motivo):
        """El corrector revisó el tramo pero lo dejó como estaba: se anota el
        veredicto y el porqué (visible como insignia en revisar.py)."""
        marcas = ",".join("?" * len(t["ids"]))
        con.execute(
            f"UPDATE participaciones SET revisado_ia='descartado', "
            f"motivo_ia=? WHERE id IN ({marcas})", [motivo] + t["ids"])
        con.commit()
    ...
    def _aplicar_correccion(t, nueva, actual, razon):
        ...
        con.execute(
            ...
            f"revisado_ia='validado', motivo_ia=? WHERE id IN ({marcas})",
            [nueva, razon] + t["ids"])
        con.commit()
        dur = t["segundos"]
    ...
    def corregir_tramo(i):
        ...
        protocolo y una voz fuerte mandan y no se tocan. Todo tramo de la
        zona gris que se evalúa queda marcado con su veredicto (validado o
        descartado) y el motivo, para que se vea en la revisión."""
        hist = estado["tramos_hist"]
        t = hist[i]
        if not t["ids"] or t["resuelto"]:
            return   # fuera del alcance del corrector (no se marca)
        actual = _label_actual(t["ids"]) or t["etiqueta"]
        if actual not in GRISES:
            return   # ya es un orador nombrado: intocable, no se marca
        emb = _huella_consolidada(t["ini"], t["fin"])
        if emb is None:
            _marcar_descartado(t, "sin audio limpio suficiente para decidir")
            return
        r = identificar_huella(emb, nombres_voz, matriz_voz, args.umbral_voz)
        if r is None:
            _marcar_descartado(t, "no se pudo comparar la voz")
            return
        ...
        if r["fuerte"] and et_voz != actual:
            _aplicar_correccion(
                t, et_voz, actual,
                f"voz consolidada {r['similitud']:.2f} sobre "
                f"{t['segundos']:.0f}s de audio")
            return
        ...
        if (et_izq and et_izq == et_der and et_izq not in GRISES
            ...
            if corto or sim_vecino >= SIM_SANDWICH:
                _aplicar_correccion(
                    t, et_izq, actual,
                    (f"micro-corte reabsorbido entre turnos de {et_izq}"
                     if corto else
                     f"turno continuo con {et_izq} (voz {sim_vecino:.2f})"))
                return

        # Descartado: se explica por qué no se cambió
        motivos = []
        if r["similitud"] < args.umbral_voz:
            motivos.append(f"la voz más parecida ({et_voz}) llega a "
                           f"{r['similitud']:.2f}, bajo el umbral "
                           f"{args.umbral_voz:.2f}")
        elif r["margen"] < MARGEN_MIN_VOZ:
            motivos.append(f"{et_voz} y {r['segundo']} casi empatan "
                           f"(margen {r['margen']:.2f})")
        else:
            motivos.append(f"la voz apunta a {et_voz} ({r['similitud']:.2f}) "
                           "pero sin apoyo del contexto")
        if intruso_claro:
            motivos.append("parece una intervención breve de "
                           f"{etiqueta_orador(ri['candidato'])}")
        if not (et_izq and et_izq == et_der):
            motivos.append("los oradores vecinos no coinciden")
        _marcar_descartado(t, "; ".join(motivos))
